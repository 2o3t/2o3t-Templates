...
const rendererConfig = require('./config/webpack.config.renderer');
...
function startRenderer() {
    return new Promise((resolve, reject) => {
        ...
        const server = new WebpackDevServer(
            compiler,
            {
                contentBase: path.join(ROOT_CWD, './'),
                quiet: true,
                before(app, ctx) {
...
function startElectron() {
    let args = [
        '--inspect=5858',
        path.join(ROOT_CWD, './build/electron/main.js'),
    ];
...
function greeting() {
    ...
    if (cols > 104) text = 'electron-2O3T';
    else if (cols > 76) text = 'electron-|2O3T';
    else text = false;
